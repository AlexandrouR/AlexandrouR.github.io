...
For further information refer to `infura`{.interpreted-text role="doc"}.

Instructions on obtaining Ether to transact on the network can be found in the`testnet section of the docs <ethereum-testnets>`{.interpreted-textrole="ref"}.
...
Then generate the wrapper code using web3j\'s `command_line`{.interpreted-text role="doc"}:
...
For more information refer to`smart-contract-wrappers`{.interpreted-text role="ref"}.
...
There are a number of other transaction and block replay Flowables described in `filters`{.interpreted-text role="doc"}.
...
For further information refer to `filters`{.interpreted-text role="doc"} and the [Web3jRx](https://github.com/web3j/web3j/blob/master/core/src/main/java/org/web3j/protocol/rx/Web3jRx.java) interface.
...
response) one would need to use the [sync]{.title-ref} method instead:
...
To subscribe to a stream of events you should use _WebSocketService_ to send an RPC method via WebSocket; this is usually _eth\_subscribe_. Events that it subscribes to depend on parameters to the _eth\_subscribe_ method. You can find more in the [RPC documentation](https://github.com/ethereum/go-ethereum/wiki/RPC-PUB-SUB#supported-subscriptions):
...
_parity\_unsubscribe_ method to unsubscribe from [pub/sub events](https://wiki.parity.io/JSONRPC-eth_pubsub-module).
...
Please refer to the `documentation <command_line>`{.interpreted-textrole="doc"} for further information.
